...
class PolicyControllerTest(
    ...
    fun `get program detail returns detail sections contacts and attachments`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.success").value(true))
            .andExpect(jsonPath("$.data.policyProgramId", equalTo(policyProgramId.toString())))
            .andExpect(jsonPath("$.data.programTitle", equalTo("청년농 정착 지원")))
            .andExpect(jsonPath("$.data.eligibilitySummary", equalTo("만 40세 미만 농업인")))
            .andExpect(jsonPath("$.data.benefitSummary", equalTo("정착지원금")))
