...
object VoiceSessionInstructions {
    ...
    private val WORK_TYPE_REQUIREMENTS = """
        ...
        - FERTILIZING(비료 주기): 사용 비료(비료명), 비료 사용량과 그 단위(g/ml) 필수. 양을 들으면 단위도 함께 확인하세요.
        - PEST_CONTROL(병해충 관리): 사용 농약(농약명), 농약 사용량과 그 단위(ml/g) 필수. 양을 들으면
          단위도 함께 확인하세요. 총 살포량도 필수지만 단위는 항상 ml이니 되묻지 말고 자동으로 채우세요.
        - HARVEST(수확): 수확량(kg) 필수. 사용자가 수확량을 모른다고 하면 harvestAmountUnknown=true로 두고
          ...
          사용자가 언급하지 않으면 반드시 물어보세요. 이번이 최종 수확인지(isLastHarvest)도 필수이니
          예/아니오로 한 번만 물어보세요. 수확 부위(medicinalPart)는 선택 사항이니 사용자가
          언급하지 않으면 굳이 묻지 마세요.
        - PLANTING(심기): 심은 방법(plantingMethod) 필수. 씨앗 심기(SEED)면 심은 씨앗량(seedAmount)만 묻되
          단위는 항상 g이니 되묻지 말고 자동으로 채우세요. 모종 심기(SEEDLING)면 심은 갯수(seedlingCount)만
          묻되 단위는 항상 주이니 되묻지 말고 자동으로 채우세요. 모종 번식법(propagationMethod)은 모종 심기일
          때만 선택적으로 물을 수 있습니다. 반대쪽 항목은 절대 채우지 마세요.

        작업 유형을 사용자에게 말할 때는 화면과 같은 이름을 쓰세요:
        심기 / 물주기 / 비료 주기 / 병해충 관리 / 잡초 관리 / 가지·순 정리 / 수확 / 기타.
    """.trimIndent()
