...
- 구형 작업 리포트 커서는 새 필수 필드가 없어 `INVALID_CURSOR`로 거절한다.
...
- Produces: `FarmingWorkReportCursorPayload.lastWorkedOn: LocalDate?`
...
`decodeCursor`에는 `lastWorkedOn = payload.lastWorkedOn`, `encodeCursor`에는 `lastWorkedOn = row.lastWorkedOn`을 추가한다.
