//
//  SearchPolicyListViewModel.swift
//  ChamChamCham
//
...
/// Drives the "정책 정보" search tab: cursor-paged, keyword-scoped. Mirrors `SearchRecordListViewModel`.
@MainActor
@Observable
final class SearchPolicyListViewModel {
    private(set) var items: [SearchPolicyItem] = []
    ...
    func reload(keyword: String?) async {
        ...
        do {
            let page = try await repository.searchPolicies(keyword: keyword, cursor: nil, size: 20)
            items = page.items
    ...
    func loadMoreIfNeeded(currentItem: SearchPolicyItem) async {
        guard let cursor = nextCursor, !isLoadingMore, !isLoading,
              currentItem.id == items.last?.id else { return }
        isLoadingMore = true
        defer { isLoadingMore = false }
        do {
            let page = try await repository.searchPolicies(keyword: keyword, cursor: cursor, size: 20)
