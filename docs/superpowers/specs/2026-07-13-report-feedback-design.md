...
>
> 새 기준 문서:
> `2026-07-14-work-type-report-feedback-design.md`
>
> 이 문서의 완료 즉시 자동 생성, `RecordFeedback`과의 도메인 분리, RAG 근거
> 원칙은 유지한다. 다만 사이클 전체를 하나의 코칭으로 생성·저장·조회하는 결정은
> 새 기준 문서가 대체한다. 아래 본문은 변경 배경을 확인하기 위한 과거 기록이며,
> 충돌하는 카디널리티·출력·API·테스트 기준에는 새 문서만 적용한다.
