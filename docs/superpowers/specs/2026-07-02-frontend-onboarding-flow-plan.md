...
- **Profile photo** — resolved and wired up (2026-07-07). The backend shipped a media upload API and connected it to onboarding: `POST /api/v1/media/images` takes a JSON body `{ usageType: "PROFILE", base64Image, originalFilename?, contentType? }` (Base64, **not** multipart) and returns `{ mediaId, imageUrl, status }`; `CompleteOnboardingRequest` gained an optional `profileMediaId: UUID?`, and `MemberProfileResponse` gained `profileImageUrl: String?`. Client flow (offline-first): the picked photo is still saved to a local file with only its filename in the draft snapshot; at final submit, `OnboardingViewModel.submit` uploads it first (via `RemoteMediaUploadRepository`, downscaling through `ImageDownscaler` to keep the Base64 payload small on poor connectivity), stores the returned `mediaId` in `draft.profileMediaId` (persisted, so a completion retry doesn't re-upload), then sends onboarding-complete. Because the photo is a "선택" field, a photo-upload failure surfaces as `SubmissionState.photoUploadFailed` offering **다시 시도 / 사진 없이 계속하기** rather than blocking the whole submission. Re-picking a photo clears any previously uploaded `profileMediaId`. See `Core/Media/*`.
