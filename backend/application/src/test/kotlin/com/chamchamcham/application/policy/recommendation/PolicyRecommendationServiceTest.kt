...
class PolicyRecommendationServiceTest {
    ...
    fun `list recommendations regenerates stale or missing member recommendations from latest sync job`() {
        ...
        `when`(policyRecommendationQueryRepository.findPage(anySearchCondition()))
    ...
    fun `list recommendations reuses latest recommendations when saved policies are a subset of candidates`() {
        ...
        verify(policyRecommendationRepository, never()).saveAll(anyPolicyRecommendations())
    }
    ...
    fun `list recommendations rejects cursor from stale sync job as invalid input`() {
        ...
        verify(policyRecommendationQueryRepository, never()).findPage(anySearchCondition())
    }
    ...
    private fun anySearchCondition(): PolicyRecommendationQueryRepository.SearchCondition {
        any<PolicyRecommendationQueryRepository.SearchCondition>()
        return PolicyRecommendationQueryRepository.SearchCondition(
            memberId = memberId,
            source = PolicySource.NONGUP_EZ,
            sourceYear = "2026",
            benefitSummary = null,
    ...
    private fun anyPolicyRecommendations(): Iterable<PolicyRecommendation> {
        any<Iterable<PolicyRecommendation>>()
        return emptyList()
    }
