...
- **빌드**: clean build 통과(iPhone 17 시뮬레이터).
- **테스트**: 전체 **118 tests / 30 suites 통과**, 회귀 없음. 신규 VM 5개 suite +
  `FarmEndpoint.delete` 경로 테스트 추가.
- **정적 코드 리뷰**: 실 이슈 1건 발견·수정 — `FarmEndpoint.delete`가 UUID를
  `.lowercased()`로 넣어 기존 컨벤션(`CommunityEndpoint`/`MemberEndpoint`의
  `id.uuidString` 대문자)과 불일치 → `id.uuidString`으로 수정 + 회귀 테스트 추가.
  (코드리뷰 보조 에이전트는 세션 한도로 중단되어 직접 리뷰로 수행.)
- **런타임 QA 미수행**: 마이페이지는 인증 게이트 뒤 → 로그인 세션에서 Figma 대조
  시각 QA 및 삭제-선택 탭 인터랙션 실기기 확인 필요.
