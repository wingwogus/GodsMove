...
/// 날씨 상세 화면(`홈 -> 날씨 상세`). `temperature`/`condition`은 `GET /farms/{id}/weather` 실데이터,
/// 나머지 필드는 배포 API에 아예 없어 더미값 사용(2026-07-14 사용자 확정 — Figma UI를 그대로 만들고
/// 백엔드가 필드를 추가하면 실데이터로 교체). 참고: docs/figma/home/2026-07-14-home-backend-conflicts.md (C-1)
struct WeatherDetail: Sendable {
    let temperature: Int
    let condition: String
    /// 아래는 전부 더미(C-1) — 값은 Figma 캡처의 예시 데이터를 그대로 사용.
    let feelsLike: Int
    let lowTemperature: Int
    let highTemperature: Int
    let uvIndexLabel: String
    ...
    static func dummy(temperature: Int, condition: String) -> WeatherDetail {
        WeatherDetail(
            ...
            humidityPercent: 45,
            windSpeedMps: 2.1,
            address: "서울 중구 퇴계로 128",
            forecast: [
                WeatherForecastDay(dayLabel: "오늘", conditionAssetName: "snowflake", temperature: 31),
                WeatherForecastDay(dayLabel: "토", conditionAssetName: "clear_day", temperature: 26),
                WeatherForecastDay(dayLabel: "일", conditionAssetName: "rainy", temperature: 27),
                WeatherForecastDay(dayLabel: "월", conditionAssetName: "cloudy", temperature: 27),
                WeatherForecastDay(dayLabel: "화", conditionAssetName: "cloud", temperature: 27),
            ]
        )
    }
}
